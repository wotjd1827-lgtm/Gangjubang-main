...
### Step 3: 사이트 소유 확인 (HTML 태그 방식 또는 HTML 파일 업로드 방식)
...
## 3. 사이트 소유확인 실패 시 트러블슈팅 Checklist

사이트 소유확인이 지속적으로 실패할 경우, 아래 5가지 원인을 순서대로 점검하고 해결합니다.

### 1️⃣ 서치어드바이저 발급 코드 불일치 (가장 흔한 원인)
- **증상**: 네이버 화면에서 새로 발급된 인증 코드와 프로젝트 내 `index.html` / `public/naver*.html` 코드가 다를 때 발생.
- **해결**:
  - 네이버 서치어드바이저 화면에 뜬 32자리 코드를 확인합니다.
  - 해당 코드로 `index.html` 17행의 `content="..."` 값과 `public/naver[코드].html` 파일명을 동기화합니다.

### 2️⃣ Git Push 및 Vercel 실서버 배포 미완료
- **증상**: 로컬 코드만 수정하고 원격 저장소(`git push`)에 올리지 않은 상태에서 네이버 서치어드바이저 [소유 확인] 클릭 시 발생.
- **해결**:
  - `git commit` 및 `git push`를 실행하여 Vercel에서 실제 배포가 완료(Deploy Success)된 후 [소유 확인] 버튼을 누릅니다.

### 3️⃣ 등록한 URL과 실제 도메인의 불일치 (HTTP/HTTPS, www 유무)
- **증상**: 서치어드바이저에는 `https://gangjubang.com`으로 등록했으나, 실제 접속 주소가 `https://www.gangjubang.com` 이거나 Vercel 서브도메인(`*.vercel.app`)인 경우.
- **해결**:
  - 서치어드바이저 사이트 등록 목록의 URL과 브라우저 주소창의 대표 URL(프로토콜 포함)이 정확히 일치해야 합니다.

### 4️⃣ HTML 태그 방식 vs HTML 파일 방식 혼용 문제
- **증상**: 서치어드바이저에서는 [HTML 파일 업로드]를 눌렀는데 프로젝트에서는 [HTML 태그]만 설정되어 있거나 그 반대의 경우.
- **해결**:
  - 서치어드바이저에서 선택한 인증 방식과 동일한 방식을 확인합니다.
  - HTML 태그 방식: `index.html` `<head>` 내 메타태그 배치
  - HTML 파일 방식: `public/naver[발급코드].html` 파일 배치

### 5️⃣ Yeti 수집 로봇 접근 차단 또는 캐시/대기 시간
- **증상**: 네이버 서버 수집 시 일시적인 네트워크 응답 지연이나 캐시 문제.
- **해결**:
  - `public/robots.txt`에 `User-agent: Yeti Allow: /` 구문이 정상 작동 중인지 확인 (현재 적용 완료).
  - Vercel 배포 완료 후 약 1~2분 뒤 다시 [소유 확인] 버튼을 클릭합니다.
